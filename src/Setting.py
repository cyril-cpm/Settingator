...
class Setting():
	...
	def GetBinaryValue(self) -> bytearray: 
		if (
				self.__name == "__RGB" and
				IsNumericalTypeValue((self.__type)) and
				isinstance(self.__value, str)
			):
			return struct.pack(PACK_TAB[self.__type], int(self.__value, 16))
		elif (IsNumericalTypeValue(self.__type)):
	...
	def SetValue(self, value):
		...
		if (self.__name == "__RGB" and IsIntegerTypeValue(self.__type)):
			if value == '':
				value = 0
		...
		elif (IsIntegerTypeValue(self.__type)):
			if value == '':
				value = 0
